...
$rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'), $arFilter, false, array('ID', 'CODE', 'NAME', 'SECTION_PAGE_URL'));
...
if (!empty($arResult['ITEMS'])) {
    foreach ($arResult['ITEMS'] as &$arItem) {
        if (!empty($arItem['PREVIEW_PICTURE'])) {
            $img = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE'], array('width' => 555, 'height' => 300), BX_RESIZE_IMAGE_PROPORTIONAL, true);
            $arItem['PREVIEW_PICTURE']['SRC'] = $img['src'];
        } else {
            $arItem['PREVIEW_PICTURE']['SRC'] = SITE_TEMPLATE_PATH . '/amika/images/news-pic-1.png';
        }
        $arItem['SECTION_EL'] = $arSection[$arItem['IBLOCK_SECTION_ID']];
    }
}